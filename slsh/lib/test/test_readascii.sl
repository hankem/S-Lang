...
private define run_test (make_lines_func)
{
   ...
   if (n != na)
     failed ("readascii returned $na, not $n as expected"$);
   if (lastlinenum != n)
     failed ("lastlinenum=$lastlinenum instead of $n"$);
   if (any (fneqs(x, xa) or fneqs(y, ya) or fneqs(z, za)))
     failed ("Arrays have unequal values");
   ...
   na = readascii (lines, &xa, &ya, &za; delim=",", lastlinenum=&lastlinenum);
   if (n+n1 != na)
     failed ("readascii returned $na, not $n1 + $n2 as expected"$);
   ...
   if (n != na)
     failed ("readascii returned $na, not $n as expected"$);
   if (lastlinenum != n+1)
     failed ("lastlinenum=$lastlinenum instead of $n+1"$);
   if (any (fneqs(x, xa) or fneqs(y, ya) or fneqs(z, za)))
     failed ("Arrays have unequal values");
   if (lastline != "@\n")
     failed ("lastline=$lastline instead of @\n");
   ...
   if (n+n1 != na)
     failed ("readascii returned $na, not return $n+$n1 as expected"$);
   if (lastlinenum != length (lines))
     failed ("lastlinenum=$lastlinenum instead of $n"$);
   if (any (fneqs([x,x1], xa) or fneqs([y,y1], ya) or fneqs([z,z1], za)))
     failed ("Arrays have unequal values");
   if (lastline != lines[-1])
     failed ("lastline=$lastline instead of lines[-1]");
   ...
   if ((typeof (za) != List_Type) || (typeof (xa) != List_Type))
     failed ("Used as_list qualifier but did not get a list, got $za,$xa"$);
